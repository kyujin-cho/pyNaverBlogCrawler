...
def dl(image, url, anum, inum, q):
    try:
        abspath = urlparse.urljoin(url, image)
        path = urlparse.urlunsplit(urlparse.urlsplit(abspath)[:3] + ('', '',))
        ...
        if path.lower().startswith('http'):
            ...
            str = urllib2.urlopen(abspath).read()
...
def getURL(post, bID):
    ...
    for i in range(1, page + 1):
        print("Try")
        print(i)
        params = urllib.urlencode({'blogId': bID, 'currentPage': i, 'countPerPage': 30})
...
def buildEpub(bookTitle, bookAuthor, bookPublisher, blogURL, count, postNo):
    ...
    for i in range(count):
        ...
        toc += '<navPoint id="navpoint-{}" playOrder="{}"> <navLabel> <text>{}</text> </navLabel> <content src="article_{}.html"/> </navPoint>'.format(
                i + 2, i + 2, cgi.escape(readable_title).encode('utf-8'), i + 1)
...
NotFoundImage = urllib2.urlopen(
        "http://learn.getgrav.org/user/pages/09.troubleshooting/01.page-not-found/error-404.png").read()
postCount = raw_input("Total post count : ")
blogID = str(raw_input("Blog ID : "))
title = str(raw_input("Book Title (default: Blog ID) : "))
postAuthor = str(raw_input("Book author (default: Blog ID) : "))
postPublisher = str(raw_input("Your name : "))
